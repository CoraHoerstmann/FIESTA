...
#NOTE: this can only be run once otherwise the dataframes are messed up
...
print(p3+
  ...
  ggtitle("PCA nifH DNA sequences") +
...
#ggplot(all3, aes(x = Sample, y = Abundance, color = Order, fill = Order))+
...
#ggplot(top_taxa, aes(x = Main_Bin, y = abundance, color = Order, fill = Order))+
#  geom_bar(stat="identity", position="stack")+
...
#PLOT
print(RDA_plots(nifHM_CTD[,c(1:21, 28:31)], ALL_CTD, nifH_ASV_ait, meta_sig))
