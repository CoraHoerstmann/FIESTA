...
ASV.ait <- nifH_ASV_ait
meta <- nifHM_CTD[,1:21]
##subset where we have the N2 data available

meta2 <- na.omit(meta)
meta2$No <- as.character(meta2$No)
ASV.clr2 <- ASV.clr%>%dplyr::select(meta2$No)
nifH_ASV_CTD2 <- nifH_ASV%>%dplyr::select(meta2$No)
nifH_ASV_ait2 <- dczm(nifH_ASV_CTD,1)


 meta_sig <- c("swell","wind_waves","sea_state","Temperature","Actual_Depth..m.")
meta_sig2 <- c("swell","wind_waves","sea_state","Temperature","Actual_Depth..m.",
              "mean_N2", "mean_CN")
...
ggords::ggrda(ASV.clr.rda, group = grl, spearrow = NULL, farrow = 0.1, fzoom = 5, ellipse = T, scaling = 1, spe = F)+
