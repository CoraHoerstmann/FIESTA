...
stats_CE_conservative_average_year$N2input_year_noEddy_eddyOcean_future <- stats_CE_conservative_average_year$Vol_year_no_eddy_future*(mean(N2_open_ocean$N2_m3))
stats_CE_conservative_average_year$N2input_year_eddy_ocean_future <- stats_CE_conservative_average_year$N2input_year_noEddy_eddyOcean_future + stats_CE_conservative_average_year$N_input_eddies_total_future
...
stats_CE_conservative_average_year_new$N2input_year_noEddy_global_estimate_future <- (((stats_CE_conservative_average_year_new$N_input_non_eddying_ocean*100*100)/(areaPolygon(df_C_minmax)*100))/1000000000)/100
stats_CE_conservative_average_year_new$N2input_year_Eddy_global_estimate_future <- (((stats_CE_conservative_average_year_new$N2input_year_eddy_ocean_future*100*100)/(areaPolygon(df_C_minmax)*100))/1000000000)/100
...
DB1 <- ggplot(data = stats_CE_conservative_average_year_longer, aes(x = scenario, y = log(N_input)))+
...
DB2 <- ggplot(data = stats_CE_average_year_longer, aes(x = scenario, y = log(N_input)))+
  geom_boxplot(aes(fill = scenario))+
  scale_fill_manual(values = c("#69f4f4", "#37378c"))+
  #geom_boxplot(width = 0.08)+
  stat_summary(fun.y=median, geom="point", size=2, color="red")
...
stats_ACE_conservative_average_year$N_input_eddies_total <- (mean(N2_mean_all_data_eddy_AC$N2)*1000)*stats_ACE_conservative_average_year$Lifetime_mean*stats_ACE_conservative_average_year$volume_mean*stats_ACE_conservative_average_year$n
stats_ACE_conservative_average_year$N_input_eddies_total_future <- (mean(N2_mean_all_data_eddy_AC$N2)*1000)*stats_ACE_conservative_average_year$Lifetime_mean*stats_ACE_conservative_average_year$volume_mean*stats_ACE_conservative_average_year$n*1.16
...
#substract the area/time coverd by the eddies: returns you the m3 per year FUTURE
stats_ACE_conservative_average_year$Vol_year_no_eddy_future <- (areaPolygon(df_AC_minmax)*100*365)-(stats_ACE_conservative_average_year$volume_mean*stats_ACE_conservative_average_year$n*1.16*stats_ACE_conservative_average_year$Lifetime_mean)
...
#for future analysis only take from 2012
...
stats_ACE_conservative_average_year_new$PERCENT_EddyContribution_Ninput_future <- (stats_ACE_conservative_average_year_new$N2input_year_Eddy_global_estimate_future - stats_ACE_conservative_average_year_new$N2input_year_noEddy_global_estimate_future)/stats_ACE_conservative_average_year_new$N2input_year_Eddy_global_estimate_future*100
mean(stats_ACE_conservative_average_year_new$PERCENT_EddyContribution_Ninput_future)
...
DB1 <- ggplot(data = stats_ACE_conservative_average_year_longer, aes(x = scenario, y = log(N_input)))+
  geom_boxplot(aes(fill = scenario))+
  scale_fill_manual(values = c("#69f4f4", "#37378c"))+
  #geom_boxplot(width = 0.08)+
  stat_summary(fun.y=median, geom="point", size=2, color="red")
